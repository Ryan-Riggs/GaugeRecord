...
filt_gauge = loc
og = loc.geometry().buffer(2000)
lg = loc.geometry().buffer(5000)
xsections = sword.filterBounds(lg.difference(og))
...
def running(xsections):
	...
	random = final_filtered.randomColumn('random',7,'uniform')
	...
	r = r.filter(ee.Filter.gte('classification',0))
	...
	output=xsections.first().set({'rrmse':out.get('rrmse'),'nse':nse, 'slope':slope, 'yInt':b})
	return(ee.Algorithms.If(testing.size().eq(0), xsections.first().set({'rrmse':9999,'nse':-9999, 'slope':0, 'yInt':1}),output))
	#return(final_filtered.limit(20))
...
folder = 'GRDC_linearRC_performance',
